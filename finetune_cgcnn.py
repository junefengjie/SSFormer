...
#        self.dataset = CGData(self.config['task'], **self.config['dataset'], shuffle=False)
    ...
    def _load_pre_trained_weights(self, model):
        try:
            ...
            print(os.path.join(checkpoints_folder, 'model_graph_900.pth'))
            load_state = torch.load(os.path.join(checkpoints_folder, 'model_graph_900.pth'),  map_location=self.config['gpu']) 
