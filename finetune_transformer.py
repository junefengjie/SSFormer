...
from dataset.dataset_finetune_transformer import SLICES_ID_Dataset
from dataset.dataset_finetune_transformer import get_slices_token
...
#            reader = csv.reader(f)
#            self.slice_data = [row for row in reader]
        self.slice_data = pd.read_csv(self.config['dataset']['dataPath'], header=None).values
#        self.slice_data = np.array(self.slice_data)
#        print(self.slice_data)

        self.train_data, self.valid_data, self.test_data = split_data(
            self.slice_data, train_ratio = self.config['dataloader']['train_ratio'], valid_ratio = self.config['dataloader']['valid_ratio'], test_ratio = self.config['dataloader']['test_ratio'],
            randomSeed= self.config['dataloader']['randomSeed']
        )
        
        self.train_dataset = SLICES_ID_Dataset(data = self.train_data)
        self.valid_dataset = SLICES_ID_Dataset(data = self.valid_data)
        self.test_dataset = SLICES_ID_Dataset(data = self.test_data)
